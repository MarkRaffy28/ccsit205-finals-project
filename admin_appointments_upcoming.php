            ...
            <tr class="text-nowrap">
              ...
              <td class="text-center">
                <div class="d-flex gap-2">
                  <button class="complete-button btn btn-sm btn-success" data-id="<?= $row["appointment_id"] ?>">
...
<div class="modal fade p-4" id="complete_appointment" tabindex="-1">  
  <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
    <div class="modal-content rounded-4 shadow">
      <div class="modal-header border-0">
        <h5 class="modal-title w-100 text-center fw-bold">COMPLETE APPOINTMENT</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>     
      <div class="modal-body text-center">
        <form method="POST" novalidate>
          <input type="hidden" name="complete_appt_id" id="complete_appt_id">
          ...
          <div class="row mb-2 gx-3 gy-2">
            <div class="col-sm form-floating">
              <input type="number" class="form-control" id="payment_amount" name="payment_amount" placeholder="Payment Amount (Whole Number)" min="0" max="2147483647" step="1" required>
...
<script>
  ...
  document.querySelectorAll(".complete-button").forEach(btn => {
    btn.addEventListener("click", ()=> {
      document.getElementById("complete_appt_id").value = btn.dataset.id;
      
      const modal = new bootstrap.Modal(document.getElementById("complete_appointment"));
      modal.show();
    });
  });
