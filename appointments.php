  <?php
    ...
    if (isset($_POST["edit_appointment_request"])) {
      ...
      if ($curr_slot_id != $edit_appt_slot_id) {
          $stmt_check_same_slot = $conn->prepare("SELECT id FROM appointments WHERE patient_id = ? AND slot_id = ? AND status IN ('Pending', 'Approved')");
        ...
        <div class="modal-content rounded-4 shadow">
          ...
          <div class="modal-body text-center">
            <form method="POST" novalidate>
              ...
              <div class="row mb-2 gx-3 gy-2">
                <div class="col-sm form-floating">
                  <input type="date" class="form-control datepicker" id="edit_appt_date" required>
  ...
  <script>
    ...
    fetch("get_dates.php")
      ...
      .then(data => {
        ...
        dateInput.min = futureDates[0];;
        dateInput.max = futureDates[futureDates.length - 1];;
    ...
    document.querySelectorAll(".edit-button").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        document.getElementById("edit_appt_service").value = btn.dataset.serviceid;
              
        fetch("get_dates.php")
          .then(res => res.json())
          .then(data => {
            const availableDates = Object.keys(data);
            const editDateInput = document.getElementById("edit_appt_date");
            const editForm = editDateInput.closest("form");

            const today = new Date().toISOString().split("T")[0];
            const futureDates = availableDates.filter(d => d >= today);
            
            editDateInput.min = futureDates[0];;
            editDateInput.max = futureDates[futureDates.length - 1];;
            
            if (availableDates.includes(editDate)) {
              updateTimes(editDate, data[editDate]);
            }
          ...
          function updateTimes(selectedDate, availableTimes) {
            const timeSelect = document.getElementById('edit_appt_time');
            timeSelect.innerHTML = '<option value="" selected disabled>-- Select a time --</option>';
            availableTimes.forEach(slot => {
              const option = document.createElement('option');
              option.value = slot.id;
              option.textContent = `${formatTime(slot.start_time)} - ${formatTime(slot.end_time)} | Available: ${slot.slot_count}`;
              if (slot.id == editSlotId) {
